...
the **real** compiled `dig-capsule-guest` serving module. It needs two artifacts
...
2. Seed a tiny store and compile it with `dig-capsule-compiler` (once that crate
   ...
   `crates/dig-capsule-host/tests/fixtures/sample.wasm`.

3. Produce the matching content request and write it to
   `crates/dig-capsule-host/tests/fixtures/hello_request.bin`. The request bytes
...
```sh
cargo test -p dig-capsule-host --test e2e_guest -- --ignored
```
