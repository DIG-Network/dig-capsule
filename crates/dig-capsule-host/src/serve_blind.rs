...
//! ([`dig_capsule_prover::CoinsetChainSource`] against coinset.org) and a real
...
use dig_capsule_prover::{ChainSource, MockChainSource, MockProver, Prover};
...
/// fixture).
fn mock_block() -> dig_capsule_core::ChiaBlockRef {
    dig_capsule_core::ChiaBlockRef {
...
pub struct BlindServeDeps {
    ...
    /// `MockChainSource`; [`dig_capsule_prover::CoinsetChainSource`] is the real one.
...
impl BlindServeDeps {
    ...
    /// [`dig_capsule_prover::CoinsetChainSource`]) and the clock with
    ...
    pub fn with_risc0_prover(mut self, node_seed: &[u8]) -> Result<Self, HostError> {
        use dig_capsule_prover::risc0_backend::Risc0Prover;
        let peak = self
            .chain
            .get_peak()
            .map_err(|e| HostError::Wasmtime(format!("chain get_peak for risc0 prover: {e}")))?;
        let node_sk = dig_capsule_crypto::bls::SecretKey::from_seed(node_seed);
