...
use dig_capsule_prover::risc0_backend::{Risc0Prover, Risc0Verifier};
...
fn risc0_prove_verify_smoke() {
    ...
    let proof = Risc0Prover::new(sk, pk, block.clone())
        .prove(ph, &pi, &serving)
...
fn risc0_tampered_output_rejected() {
    ...
    let mut proof = Risc0Prover::new(sk, pk, block.clone())
        .prove(ph, &pi, &serving)
        .unwrap();
