...
use dig_capsule_core::{Bytes32, ChiaBlockRef};
use dig_capsule_crypto::bls;
use dig_capsule_prover::build_public_input;
use dig_capsule_prover::hardware::{HardwareAttestProver, HardwareVerifier};
use dig_capsule_prover::{MockChainSource, Prover, ProverError, ServingInputs, Verifier};
...
fn hardware_attest_round_trip() {
    ...
    let proof = HardwareAttestProver::new(node_sk, node_pk, enclave_sk, block.clone())
        .prove(ph, &pi, &serving)
        .unwrap();
    let chain = MockChainSource::new(vec![block], 1_000_100);
    HardwareVerifier::new(enclave_pk.to_bytes())
...
fn tampered_attestation_rejected() {
    let (node_sk, node_pk, enclave_sk, enclave_pk, block, ph, root, pi, serving) = fixture();
    let mut proof = HardwareAttestProver::new(node_sk, node_pk, enclave_sk, block.clone())
        .prove(ph, &pi, &serving)
        .unwrap();
    proof.proof[0] ^= 0xFF;
    let chain = MockChainSource::new(vec![block], 1_000_100);
    let err = HardwareVerifier::new(enclave_pk.to_bytes())
        .verify(&proof, ph, &[root], &chain)
        .unwrap_err();
...
fn wrong_enclave_key_rejected() {
    ...
    let proof = HardwareAttestProver::new(node_sk, node_pk, enclave_sk, block.clone())
        ...
        .unwrap();
    let chain = MockChainSource::new(vec![block], 1_000_100);
    let wrong = bls::SecretKey::from_seed(&[7u8; 32])
        .public_key()
        .to_bytes();
    let err = HardwareVerifier::new(wrong)
        .verify(&proof, ph, &[root], &chain)
        .unwrap_err();
    assert!(matches!(err, ProverError::AttestationInvalid(_)));
}
