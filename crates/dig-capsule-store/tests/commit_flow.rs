use dig_capsule_core::{Bytes32, StoreConfig, Visibility};
use dig_capsule_store::{FixedClock, StagingArea, Store};
use std::io::Write;
use tempfile::tempdir;

/// The canonical chunker config `Store::commit` uses (mirror for tests).
fn test_chunker() -> dig_capsule_core::ChunkerConfig {
    dig_capsule_core::ChunkerConfig {
        min_size: 16 * 1024,
        target_size: 64 * 1024,
        max_size: 256 * 1024,
        mask: (1u64 << 16) - 1,
...
fn expected_resource_leaves(
    store_id: Bytes32,
    salt: Option<&dig_capsule_core::SecretSalt>,
    staged: &[(&str, &[u8])],
) -> Vec<Bytes32> {
    ...
    let mut keyed: Vec<([u8; 32], Bytes32)> = staged
        ...
        .map(|(rk, content)| {
            ...
            let aes_key = dig_capsule_crypto::derive_decryption_key(&urn.canonical(), salt);
            ...
            (urn.retrieval_key().0, dig_capsule_crypto::sha256(&blob))
        })
        .collect();
...
fn add_rejects_file_outside_base() {
    ...
    assert!(matches!(err, dig_capsule_store::StoreError::PathEscape(_)));
}
...
fn commit_creates_generation_and_advances_history() {
    ...
    assert!(
        dig_capsule_store::StagingArea::open(store.paths().staging_file())
            .unwrap()
            .is_empty()
            .unwrap()
    );
...
fn commit_generation_root_equals_recomputed_tree_root() {
    ...
    use dig_capsule_core::serving::concat_output;
    use dig_capsule_core::Bytes32 as CoreBytes32;
    ...
    let mut keyed: Vec<([u8; 32], CoreBytes32)> = manifest
        ...
        .map(|kt| {
            ...
            let blob = concat_output(&slices);
            (kt.static_key.0, dig_capsule_crypto::sha256(&blob))
        })
        .collect();
    keyed.sort_by(|a, b| a.0.cmp(&b.0));
...
fn commit_refuses_empty_staging() {
    ...
    assert!(matches!(err, dig_capsule_store::StoreError::EmptyStaging));
}
...
fn resolve_chunk_reads_deduplicated_chunk_across_generations() {
    ...
    assert_eq!(dig_capsule_crypto::sha256(&bytes), shared);
}
...
fn diff_unknown_root_errors() {
    ...
    assert!(matches!(
        err,
        dig_capsule_store::StoreError::GenerationNotFound(_)
    ));
}
