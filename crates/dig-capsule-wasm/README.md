...
`dig-capsule-crypto` is enforced by `tests/parity.rs`):
...
## Why a separate crate (not `dig-capsule-crypto` directly)
...
`tests/parity.rs` cross-checks every formula against the real `dig-capsule-crypto`.
...
```sh
cargo test --test parity                       # native: byte-parity vs dig-capsule-crypto
