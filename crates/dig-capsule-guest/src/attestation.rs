...
/// `dig_capsule_crypto::bls::attestation_signing_message` (both reference the same
/// `dig_capsule_core::ATTEST_DST`): the host signs exactly these bytes and the guest
/// verifies exactly these bytes.
pub fn build_challenge(nonce: [u8; 32], store_id: [u8; 32], time: u64) -> Vec<u8> {
    use dig_capsule_core::ATTEST_DST;
