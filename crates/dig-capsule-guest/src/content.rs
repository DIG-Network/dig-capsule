...
use dig_capsule_core::MerkleProof;
...
use dig_capsule_core::{Bytes32, ContentResponse};
...
fn embedded_trusted_set(ds: &DataSection) -> crate::attestation::TrustedSet {
    use dig_capsule_core::codec::Decoder;
...
fn build_real_proof(
    ds: &DataSection,
    entry: &dig_capsule_core::KeyTableEntry,
    root: &Bytes32,
) -> MerkleProof {
    use dig_capsule_core::datasection::decode_merkle_leaves;
...
fn resource_leaf_index(ds: &DataSection, served_key: &Bytes32) -> usize {
    ...
    let mut dec = dig_capsule_core::codec::Decoder::new(body);
    let count = match u32::decode(&mut dec) {
        Ok(c) => c,
        Err(_) => return 0,
    };
    let mut rank = 0usize;
    for _ in 0..count {
        match dig_capsule_core::KeyTableEntry::decode(&mut dec) {
            Ok(e) => {
...
/// canonical [`dig_capsule_core::AuthenticationInfo`]. Returns `None` if the section
/// is absent or malformed (the gate then treats JWT as not-required / no JWKS).
fn embedded_auth_info(ds: &DataSection) -> Option<dig_capsule_core::AuthenticationInfo> {
    use dig_capsule_core::codec::Decoder;
    let body = ds.section(SectionId::AuthInfo)?;
    let mut dec = Decoder::new(body);
    dig_capsule_core::AuthenticationInfo::decode(&mut dec).ok()
}
