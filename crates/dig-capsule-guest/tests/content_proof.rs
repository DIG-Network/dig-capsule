...
use dig_capsule_guest::content::emit_merkle_proof;
...
mod fixtures;
mod mock_host;
use dig_capsule_core::{ContentResponse, KeyTableEntry};
use dig_capsule_guest::content::{serve_content, ContentOutcome, GateConfig};
use dig_capsule_guest::datasection::{encode_key_table, DataSection, SectionId};
use dig_capsule_guest::request::{ContentRequest, ValidityWindow};
...
use dig_capsule_core::codec::{Encode, Encoder};
use dig_capsule_guest::datasection::encode_blob;
...
struct SigningHost {
    secret: dig_capsule_crypto::bls::SecretKey,
    pubkey: [u8; 48],
    time: u64,
...
impl SigningHost {
    fn new(seed: &[u8; 32]) -> Self {
        let secret = dig_capsule_crypto::bls::SecretKey::from_seed(seed);
...
impl dig_capsule_guest::host::DigHost for SigningHost {
    fn get_public_key(&self) -> dig_capsule_guest::host::HostResult {
        Ok(self.pubkey.to_vec())
    }
    fn create_attestation(&self, challenge: &[u8]) -> dig_capsule_guest::host::HostResult {
        // Sign the EXACT challenge bytes the gate handed us (AugScheme).
        let mut sig = dig_capsule_crypto::bls::bls_sign(&self.secret, challenge).0;
        if self.corrupt_sig {
    ...
    fn establish_session(&self, _c: &[u8]) -> dig_capsule_guest::host::HostResult {
        Ok(vec![1u8; 16])
    }
    fn verify_session(&self) -> bool {
        true
    }
    fn jwks_fetch(&self, _u: &[u8]) -> dig_capsule_guest::host::HostResult {
        Ok(b"{}".to_vec())
    }
    fn current_time(&self) -> u64 {
        self.time
    }
    fn random_bytes(&self, count: u32) -> dig_capsule_guest::host::HostResult {
        let n = self.rand.get();
        self.rand.set(n + 1);
        Ok((0..count)
...
fn attest_fixture(corrupt: bool, trusted_includes_signer: bool) -> ContentOutcome {
    ...
    let trusted: Vec<[u8; 48]> = if trusted_includes_signer {
        ...
        let other = dig_capsule_crypto::bls::SecretKey::from_seed(&[7u8; 32]);
...
fn failed_attestation_returns_decoy() {
    ...
    let ds = DataSection::parse(&blob).unwrap();
    let host = MockHost {
        attestation: Err(dig_capsule_core::ErrorCode::AttestationFailed),
        ..MockHost::default()
    };
    let mut gc = gate_config();
    gc.require_attestation = true;
...
use dig_capsule_core::ProofPrelude;
use dig_capsule_guest::proof::{serve_proof, ProofOutcome};
use dig_capsule_guest::request::ProofRequest;
...
use dig_capsule_guest::content::verify_request_jwt;
use dig_capsule_guest::jwt::ClaimPolicy;
...
fn section_with_authinfo(
    ...
    use dig_capsule_core::AuthenticationInfo;
...
impl dig_capsule_guest::host::DigHost for JwksHost {
    fn get_public_key(&self) -> dig_capsule_guest::host::HostResult {
        Ok(vec![0xAB; 48])
    }
    fn create_attestation(&self, _c: &[u8]) -> dig_capsule_guest::host::HostResult {
        Ok(vec![0u8; 176])
    }
    fn establish_session(&self, _c: &[u8]) -> dig_capsule_guest::host::HostResult {
        Ok(vec![1u8; 16])
