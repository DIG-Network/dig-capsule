mod mock_host;
use dig_capsule_core::ErrorCode;
use dig_capsule_guest::session::{ensure_session, gated_jwks_fetch};
...
fn ensure_session_establishes_when_absent() {
    ...
    struct H;
    impl dig_capsule_guest::host::DigHost for H {
        ...
        fn random_bytes(&self, c: u32) -> dig_capsule_guest::host::HostResult {
            Ok(vec![1; c as usize])
        }
    }
    let h = H;
...
use dig_capsule_guest::jwt::{check_claims, decode_unverified, ClaimPolicy, JwtError, JwtParts};
...
use dig_capsule_guest::jwt::{verify_signature, Jwk};
