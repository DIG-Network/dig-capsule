...
use dig_capsule_guest::decoy::{decoy_bytes, decoy_size};
...
fn same_key_same_bytes() {
    ...
    assert_eq!(
        a, b,
        "decoy bytes must be deterministic for a fixed retrieval key"
    );
}
...
fn size_is_deterministic_and_in_log_band() {
    ...
    assert!(
        (1024..=256 * 1024).contains(&s1),
        "size {s1} out of log band"
    );
    assert_eq!(
        decoy_bytes(&k).len(),
        s1,
        "byte length must equal decoy_size"
    );
}
...
fn distribution_spreads_across_buckets() {
    ...
    assert!(
        sizes.len() > 10,
        "expected varied decoy sizes, got {}",
        sizes.len()
    );
}

use dig_capsule_core::{ContentResponse, MerkleProof, ProofStep};
use dig_capsule_guest::decoy::decoy_content_response;
...
fn decoy_content_response_has_real_field_shape() {
    ...
    assert_eq!(
        resp.roothash, root,
        "decoy must carry the requested/current root"
    );
    assert_eq!(
        resp.ciphertext,
        decoy_content_response(&k, &root).ciphertext,
        "deterministic"
    );
    ...
    assert!(
        !p.path.is_empty(),
        "decoy proof must have a path like a real one"
    );
