...
use dig_capsule_core::{Bytes32, KeyTableEntry};
use dig_capsule_guest::attestation::build_challenge;
use dig_capsule_guest::content::{serve_content, ContentOutcome, GateConfig};
use dig_capsule_guest::datasection::{encode_blob, encode_key_table, DataSection, SectionId};
use dig_capsule_guest::host::{DigHost, HostResult};
use dig_capsule_guest::request::ContentRequest;
...
impl DigHost for CapturingSigningHost {
    ...
    fn create_attestation(&self, challenge: &[u8]) -> HostResult {
        ...
        *self.captured.borrow_mut() = Some(challenge.to_vec());
        ...
        let sig = dig_capsule_crypto::bls::bls_sign(&self.secret, msg).0;
...
fn gate_signs_real_challenge_built_from_nonce_store_id_time() {
    ...
    let tl = dig_capsule_core::ATTEST_DST.len();
    assert_eq!(
        captured.len(),
        tl + 72,
