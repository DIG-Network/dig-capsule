...
fn fixture_chain_matches_canonical_chain_constant() {
    ...
    assert_eq!(
        set.canonical_chain,
        dig_capsule_core::CHAIN,
        "the fixture's canonical chain must equal dig_capsule_core::CHAIN"
    );
    assert_eq!(
        set.canonical_chain,
...
fn regenerate() {
    ...
    let doc = serde_json::json!({
        "_comment": "FROZEN normative URN conformance vectors (gap #128). Grammar: dig_capsule_core::urn_grammar (URN_ABNF). Regenerate via `cargo test -p dig-capsule-core --test urn_conformance -- --ignored regenerate`. retrieval_key_hex = SHA-256(canonical) lowercase hex.",
        "canonical_chain": dig_capsule_core::CHAIN,
        "valid": valid,
        "invalid": invalid,
    });
