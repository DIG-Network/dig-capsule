...
//! big-endian Chia framing used by the rest of `dig-capsule-core`). The signing
//! message (with its per-role domain-separation tag) and the BLS sign/verify
//! live in `dig-capsule-crypto`, mirroring the push/node/attestation split.
...
/// `dig_capsule_crypto::sign_tombstone` and carried alongside the record on the
/// wire.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
