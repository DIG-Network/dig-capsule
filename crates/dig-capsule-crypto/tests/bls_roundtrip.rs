...
fn from_bytes_rejects_non_canonical_public_key() {
    ...
    use dig_capsule_crypto::CryptoError;
    ...
    assert_eq!(
        err,
        CryptoError::Bls(dig_capsule_crypto::BlsError::InvalidPublicKey)
    );
}
...
fn sign_then_verify_round_trip_free_helpers() {
    use dig_capsule_crypto::{bls_sign, bls_verify};
...
fn verify_rejects_malformed_signature_bytes() {
    use dig_capsule_core::Bytes96;
    use dig_capsule_crypto::bls_verify;
...
fn verify_rejects_malformed_public_key_bytes() {
    use dig_capsule_core::{Bytes48, Bytes96};
    use dig_capsule_crypto::{bls_sign, bls_verify};
...
fn chia_aug_scheme_known_vector() {
    ...
    assert!(dig_capsule_crypto::bls_verify(&pk48, &msg, &sig96));
    // And must NOT verify a different message (binding sanity).
    assert!(!dig_capsule_crypto::bls_verify(&pk48, &[9u8, 9, 9], &sig96));
}

#[test]
fn sign_push_then_verify_push_round_trip_and_binding() {
    use dig_capsule_core::Bytes32;
    use dig_capsule_crypto::{push_signing_message, sha256, sign_push, verify_push};
    ...
    concat.extend_from_slice(dig_capsule_crypto::bls::PUSH_DST);
...
fn sign_node_binds_program_output_anchor_and_input() {
    use dig_capsule_core::Bytes32;
    use dig_capsule_crypto::{bls_verify, node_signing_message, sign_node};
...
fn node_signing_message_layout_is_exact() {
    use dig_capsule_core::Bytes32;
    use dig_capsule_crypto::node_signing_message;
    ...
    let t = dig_capsule_crypto::bls::NODE_DST;
    let tl = t.len();
...
fn sign_attestation_binds_nonce_store_and_timestamp() {
    use dig_capsule_core::AttestationChallenge;
    use dig_capsule_crypto::{attestation_signing_message, bls_verify, sign_attestation};
    ...
    let t = dig_capsule_core::ATTEST_DST;
    let tl = t.len();
...
fn role_tags_domain_separate_identical_payloads() {
    ...
    assert_eq!(
        &attest_msg[..dig_capsule_core::ATTEST_DST.len()],
        dig_capsule_core::ATTEST_DST
    );
    ...
    let challenge = dig_capsule_core::AttestationChallenge {
        nonce: [0x11u8; 32],
        store_id: [0x22u8; 32],
        timestamp: 0,
    };
...
fn sign_tombstone_then_verify_round_trip_and_binding() {
    use dig_capsule_core::tombstone::{RevocationReason, Tombstone};
    use dig_capsule_core::Bytes32;
    use dig_capsule_crypto::{sha256, sign_tombstone, tombstone_signing_message, verify_tombstone};
    ...
    let mut concat = Vec::new();
    concat.extend_from_slice(dig_capsule_crypto::bls::TOMB_DST);
    ...
    let bogus = dig_capsule_core::Bytes96([0xFFu8; 96]);
...
fn tombstone_message_domain_separated_from_other_roles() {
    ...
    let tomb = dig_capsule_crypto::bls::TOMB_DST;
    assert_ne!(tomb, dig_capsule_crypto::bls::PUSH_DST);
    assert_ne!(tomb, dig_capsule_crypto::bls::NODE_DST);
    assert_ne!(tomb, dig_capsule_core::ATTEST_DST);
    ...
    let push_sig = dig_capsule_crypto::sign_push(&sk, &root, &store_id);
...
fn verify_push_ok(
    sk: &bls::SecretKey,
    root: &dig_capsule_core::Bytes32,
    store: &dig_capsule_core::Bytes32,
    sig: &dig_capsule_core::Bytes96,
) -> bool {
    dig_capsule_crypto::verify_push(&sk.public_key(), root, store, sig)
}
