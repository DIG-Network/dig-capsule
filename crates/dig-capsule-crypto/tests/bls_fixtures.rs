use dig_capsule_crypto::fixtures::BlsFixtureSet;
...
fn fixture_set_self_verifies_and_tags_scheme() {
    ...
    assert_eq!(
        set.scheme,
        dig_capsule_crypto::CHIA_BLS_SCHEME,
        "scheme tag must be the shared const"
    );
    assert!(!set.vectors.is_empty(), "must emit at least one vector");
    for v in &set.vectors {
        let pk = dig_capsule_core::Bytes48(hex::decode(&v.pubkey_hex).unwrap().try_into().unwrap());
        let sig =
            dig_capsule_core::Bytes96(hex::decode(&v.signature_hex).unwrap().try_into().unwrap());
        let msg = hex::decode(&v.message_hex).unwrap();
        assert!(
            dig_capsule_crypto::bls_verify(&pk, &msg, &sig),
...
fn committed_bls_fixture_matches_generated() {
    ...
    let on_disk = std::fs::read_to_string(&path).expect(
        "committed bls_vectors.json must exist; run: cargo run -p dig-capsule-crypto --example gen_fixtures",
    );
...
fn write_path_is_idempotent_in_tempdir() {
    ...
    dig_capsule_crypto::write_bls_fixtures(&path).expect("write fixtures to tempdir");
    let first = std::fs::read_to_string(&path).unwrap();
    dig_capsule_crypto::write_bls_fixtures(&path).expect("rewrite is deterministic");
