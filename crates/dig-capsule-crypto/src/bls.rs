...
//! [`Signature`] (G1 = 48 bytes, G2 = 96 bytes). `dig-capsule-host` and
//! `dig-capsule-prover` consume these types (with the [`BlsSecretKey`] /
...
use dig_capsule_core::{AttestationChallenge, Bytes32, Bytes48, Bytes96, Tombstone};
...
/// Alias used by `dig-capsule-host` / `dig-capsule-prover` (CONVENTIONS C1).
...
/// `dig_capsule_core::merkle::NODE_TAG`. This is safe — it is not a cross-domain
...
/// from `dig_capsule_core` so the producer here and the guest's `build_challenge`
/// verifier share ONE definition and stay byte-identical.
pub use dig_capsule_core::ATTEST_DST;
...
mod request_auth_tests {
    ...
    fn bls_role_dsts_are_pairwise_distinct_and_node_tag_coincidence_is_intentional() {
        ...
        assert_eq!(
            NODE_DST,
            dig_capsule_core::merkle::NODE_TAG,
        ...
        for (name, dst) in role_dsts {
            assert_ne!(
                dst,
                dig_capsule_core::merkle::LEAF_TAG,
