...
//! wasm as the template), then drive it through `dig_capsule_host::HostRuntime` and
...
fn real_compiled_module_serves_itself_with_verifying_proof() {
    ...
    let opened_a = dig_capsule_crypto::decrypt_chunk(&key, &ct_a).expect("chunk A opens");
    let opened_b = dig_capsule_crypto::decrypt_chunk(&key, &ct_b).expect("chunk B opens");
