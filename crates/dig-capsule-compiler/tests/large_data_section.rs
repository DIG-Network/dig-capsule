...
//! real `.wasm` from the actual `dig-capsule-guest` wasm template, drives it through
//! `dig_capsule_host::HostRuntime`, and asserts:
...
use dig_capsule_prover::{MockChainSource, MockProver};
...
/// no JWT, no window (matches `dig_capsule_guest::request::ContentRequest::encode`).
...
fn serve_single_resource_and_verify(payload: Vec<u8>, tag: &str, uniform_blob_len: usize) -> u64 {
    ...
    let opened = dig_capsule_crypto::decrypt_chunk(&key, &ciphertext).expect("chunk opens");
    assert_eq!(
        opened, payload,
...
fn compile_and_extract_root(uniform_blob_len: usize, tag: &str) -> (u64, Bytes32) {
    ...
    let identity = dig_capsule_compiler::verify_module_root(&module, &store_id)
...
fn near_cap_store_serves_within_the_384mib_ceiling() {
    ...
    // (`dig_capsule_compiler::FIXED_BLOB_LEN`) and served end-to-end through the
    ...
    serve_single_resource_and_verify(
        payload,
        "near-cap",
        dig_capsule_compiler::FIXED_BLOB_LEN, // full 128 MiB production budget
    );
}
