...
//!   streamable framing via `dig_capsule_core::Encode`), not the paper's
...
//! - **Stats (CONVENTIONS C6):** the canonical [`dig_capsule_core::CompilationStats`]
//!   is reused for [`dig_capsule_core::CompilationResult::stats`]; the compiler's
...
//! The byte-exact data-section format is owned by `dig_capsule_core::datasection`.
//! The compiler emits the blob via that module and injects it as an ACTIVE data
//! segment at [`DATA_SECTION_MEM_OFFSET`] (= `dig_capsule_core::datasection::DIGS_DATA_OFFSET`,
...
pub use dig_capsule_core::{CompilationResult, CompilationStats};
