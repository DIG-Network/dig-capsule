...
/// [`dig_capsule_core::datasection::DIGS_DATA_OFFSET`] (2 MiB). The compiler injects
...
///
/// `result` is the canonical [`dig_capsule_core::CompilationResult`] (its `stats`
/// field is the canonical [`dig_capsule_core::CompilationStats`]). `detail` is the
...
impl Compiler {
    ...
    pub fn compile<G: GenerationView>(
        ...
        chain_state: Option<dig_capsule_core::datasection::ChainState>,
        ...
        if blob_len_without_filler > config.uniform_blob_len {
            // Cannot happen under `dig_capsule_core::MAX_STORE_BYTES` with the
