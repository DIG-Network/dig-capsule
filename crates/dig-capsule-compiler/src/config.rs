...
/// the DIGS blob header byte (== 1, owned by `dig_capsule_core::datasection`).
...
/// covers a max-cap store (`dig_capsule_core::MAX_STORE_BYTES` = 128 MB = 122.07
/// MiB) ciphertext + key table + merkle + header with ~6 MiB headroom.
///
/// CANONICAL capsule-size relationship (#130): `dig_capsule_core::MAX_STORE_BYTES`
...
const _: () = assert!(
    FIXED_BLOB_LEN as u64 >= dig_capsule_core::MAX_STORE_BYTES,
    "FIXED_BLOB_LEN must be >= dig_capsule_core::MAX_STORE_BYTES (the canonical \
...
pub struct CompilerConfig {
    ...
    /// `dig_capsule_core::MAX_STORE_BYTES`.
...
/// with the canonical [`dig_capsule_core::CompilationStats`] carried inside
/// [`dig_capsule_core::CompilationResult`]).
