...
/// Maximum linear-memory pages the served module may declare (§5.1: 384 MiB
/// ceiling). Sized to hold the fixed ~128 MiB uniform data region PLUS a
/// serve-time heap copy of a worst-case single max-size resource
/// (`dig_capsule_core::serving::concat_output` copies the resource's chunk
/// ciphertexts into a fresh `Vec`, up to ~122 MiB) PLUS response framing
/// headroom. Derived from `dig_capsule_core::MAX_STORE_BYTES`.
...
pub fn load_template(bytes: &[u8]) -> Result<Template> {
    ...
    // §5.1: a DECLARED maximum must not exceed the 384 MiB ceiling. A raw guest
    // template (rustc/LLVM output) may legitimately declare NO maximum; the
    // compiler normalizes the EMITTED module to `Some(6144)` during injection
...
/// MUST declare `maximum: Some(6144)` exactly (384 MiB). Unlike [`load_template`]
...
mod tests {
    ...
    fn emitted_module_without_declared_memory_max_fails_ceiling_check() {
        // §5.1: the served module MUST declare `maximum: Some(6144)`.
        let bytes = full_abi_module(r#"(memory (export "memory") 1)"#);
    ...
    fn emitted_module_with_memory_max_under_ceiling_fails_ceiling_check() {
        // §5.1: the module-declared cap is EXACTLY 6144 pages (384 MiB).
    ...
    fn template_with_memory_max_over_ceiling_is_rejected() {
        // Full ABI but max pages 6145 (> 6144) -> rejected.
        let watsrc = r#"(module
