...
//! wasm, so the module serves itself through `dig_capsule_host::HostRuntime`).
//!
...
/// genuinely self-serving through `dig_capsule_host::HostRuntime::serve_content`
...
pub fn build_prepared(
    ...
    for (resource_key, content) in files {
        ...
        let chunk_cts: Vec<Vec<u8>> = if pre_encrypted {
            ...
            chunks
                .iter()
                .map(|c| dig_capsule_crypto::encrypt_chunk(&aes_key, &c.data))
                .collect()
        };
        let mut indices = Vec::with_capacity(chunk_cts.len());
        for ct in &chunk_cts {
            let h = dig_capsule_crypto::sha256(ct);
        ...
        let resource_blob = dig_capsule_core::serving::concat_output(&slices);
        keyed_leaves.push((
            urn.retrieval_key().0,
            dig_capsule_crypto::sha256(&resource_blob),
        ));
...
impl CompiledCapsule {
    ...
    /// (= `dig_capsule_core::Capsule::canonical()`).
...
pub fn finalize(
    ...
    let public_manifest = if opts.include_public_manifest {
        Some(
            dig_capsule_store::build_public_manifest(&generations_dir)
...
fn compile_module(
    ...
    root: Bytes32,
    opts: &FinalizeOptions,
    public_manifest: Option<dig_capsule_core::PublicManifest>,
) -> Result<PathBuf, StageError> {
    use dig_capsule_compiler::{Compiler, CompilerConfig, GenerationView, ResourceView};
        ...
        .map(|kt| Res {
            ...
            chunks: kt
                ...
                .map(|&i| {
                    ...
                    (dig_capsule_crypto::sha256(&body), body)
                })
                .collect(),
        })
        .collect();
...
mod tests {
    ...
    fn trusted_pubkey() -> (Vec<TrustedHostKey>, Bytes48) {
        ...
        let secret = dig_capsule_crypto::bls::SecretKey::from_seed(&[7u8; 32]);
    ...
    fn public_store_embeds_public_manifest_section() {
        ...
        let blob = dig_capsule_compiler::extract_data_section_blob(&module).unwrap();
        let pm = dig_capsule_core::datasection::read_public_manifest(&blob)
            .unwrap()
    ...
    fn private_store_omits_public_manifest_section() {
        ...
        let module = std::fs::read(&cap.module_path).unwrap();
        let blob = dig_capsule_compiler::extract_data_section_blob(&module).unwrap();
        assert!(dig_capsule_core::datasection::read_public_manifest(&blob)
            .unwrap()
            .is_none());
    }
