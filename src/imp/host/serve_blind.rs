...
//! ([`crate::imp::prover::CoinsetChainSource`] against coinset.org) and a real
//! wall clock ([`crate::imp::host::clock::SystemClock`]) with no extra dependencies, and
...
use crate::imp::prover::{ChainSource, MockChainSource, MockProver, Prover};

use crate::imp::host::clock::{Clock, FixedClock};
use crate::imp::host::config::ExecutionLimits;
use crate::imp::host::error::HostError;
use crate::imp::host::runtime::{HostDeps, HostRuntime};
...
/// fixture).
fn mock_block() -> crate::imp::core::ChiaBlockRef {
    crate::imp::core::ChiaBlockRef {
...
pub struct BlindServeDeps {
    ...
    /// `MockChainSource`; [`crate::imp::prover::CoinsetChainSource`] is the real one.
...
impl BlindServeDeps {
    ...
    /// [`crate::imp::prover::CoinsetChainSource`]) and the clock with
    /// [`crate::imp::host::clock::SystemClock`]. The prover is left as-is (still the mock
    ...
    pub fn with_real_chain_clock(mut self, chain: Arc<dyn ChainSource>) -> Self {
        self.chain = chain;
        self.clock = Arc::new(crate::imp::host::clock::SystemClock);
        self
    }
    ...
    pub fn with_risc0_prover(mut self, node_seed: &[u8]) -> Result<Self, HostError> {
        use crate::imp::prover::risc0_backend::Risc0Prover;
        let peak = self
            .chain
            .get_peak()
            .map_err(|e| HostError::Wasmtime(format!("chain get_peak for risc0 prover: {e}")))?;
        let node_sk = crate::imp::crypto::bls::SecretKey::from_seed(node_seed);
