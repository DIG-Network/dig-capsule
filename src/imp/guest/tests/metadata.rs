...
use crate::imp::guest::datasection::DataSection;
use crate::imp::guest::metadata::{current_roothash, metadata_bytes, public_key, store_id};

#[test]
fn store_id_and_root_come_from_section() {
    let blob = super::fixtures::build_minimal_section([0x5A; 32], [0x6B; 32], &[]);
...
fn metadata_is_returned_verbatim_and_ungated() {
    ...
    let blob = super::fixtures::section_with_metadata([1; 32], [2; 32], manifest);
    let ds = DataSection::parse(&blob).unwrap();
...
fn public_key_is_48_bytes() {
    let blob = super::fixtures::section_with_pubkey([1; 32], [2; 32], &[0xCD; 48]);
    let ds = DataSection::parse(&blob).unwrap();
