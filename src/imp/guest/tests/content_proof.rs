...
use crate::imp::guest::content::emit_merkle_proof;
...
use crate::imp::guest::datasection::encode_blob;
...
impl crate::imp::guest::host::DigHost for SigningHost {
    fn get_public_key(&self) -> crate::imp::guest::host::HostResult {
        Ok(self.pubkey.to_vec())
    }
    fn create_attestation(&self, challenge: &[u8]) -> crate::imp::guest::host::HostResult {
        // Sign the EXACT challenge bytes the gate handed us (AugScheme).
        let mut sig = crate::imp::crypto::bls::bls_sign(&self.secret, challenge).0;
        if self.corrupt_sig {
    ...
    fn establish_session(&self, _c: &[u8]) -> crate::imp::guest::host::HostResult {
        Ok(vec![1u8; 16])
    }
    fn verify_session(&self) -> bool {
        true
    }
    fn jwks_fetch(&self, _u: &[u8]) -> crate::imp::guest::host::HostResult {
        Ok(b"{}".to_vec())
    }
    fn current_time(&self) -> u64 {
        self.time
    }
    fn random_bytes(&self, count: u32) -> crate::imp::guest::host::HostResult {
        let n = self.rand.get();
        self.rand.set(n + 1);
        Ok((0..count)
...
fn attest_fixture(corrupt: bool, trusted_includes_signer: bool) -> ContentOutcome {
    ...
    let trusted: Vec<[u8; 48]> = if trusted_includes_signer {
        ...
        let other = crate::imp::crypto::bls::SecretKey::from_seed(&[7u8; 32]);
...
use crate::imp::core::ProofPrelude;
...
use crate::imp::guest::content::verify_request_jwt;
use crate::imp::guest::jwt::ClaimPolicy;
...
fn section_with_authinfo(
    ...
    use crate::imp::core::AuthenticationInfo;
...
impl crate::imp::guest::host::DigHost for JwksHost {
    fn get_public_key(&self) -> crate::imp::guest::host::HostResult {
        Ok(vec![0xAB; 48])
    }
    fn create_attestation(&self, _c: &[u8]) -> crate::imp::guest::host::HostResult {
        Ok(vec![0u8; 176])
    }
    fn establish_session(&self, _c: &[u8]) -> crate::imp::guest::host::HostResult {
        Ok(vec![1u8; 16])
