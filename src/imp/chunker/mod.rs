...
pub use chunker::{chunk_slice, Chunker};
// `chunk_stream` reads from `std::io::Read`, so it is only available with `std`
// (the collapsed crate is no_std under `--no-default-features` / the guest-wasm build).
#[cfg(feature = "std")]
pub use chunker::chunk_stream;
