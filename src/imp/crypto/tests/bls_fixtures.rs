use crate::imp::crypto::fixtures::BlsFixtureSet;
...
fn fixture_set_self_verifies_and_tags_scheme() {
    ...
    assert_eq!(
        set.scheme,
        crate::imp::crypto::CHIA_BLS_SCHEME,
        "scheme tag must be the shared const"
    );
    assert!(!set.vectors.is_empty(), "must emit at least one vector");
    for v in &set.vectors {
        let pk = crate::imp::core::Bytes48(hex::decode(&v.pubkey_hex).unwrap().try_into().unwrap());
        let sig =
            crate::imp::core::Bytes96(hex::decode(&v.signature_hex).unwrap().try_into().unwrap());
        let msg = hex::decode(&v.message_hex).unwrap();
        assert!(
            crate::imp::crypto::bls_verify(&pk, &msg, &sig),
...
fn write_path_is_idempotent_in_tempdir() {
    ...
    crate::imp::crypto::write_bls_fixtures(&path).expect("write fixtures to tempdir");
    let first = std::fs::read_to_string(&path).unwrap();
    crate::imp::crypto::write_bls_fixtures(&path).expect("rewrite is deterministic");
