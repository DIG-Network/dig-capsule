use crate::imp::crypto::fixtures::KdfFixtureSet;
...
fn derive_decryption_key_public_is_32_bytes_and_deterministic() {
    ...
    let k1 = crate::imp::crypto::derive_decryption_key(canonical, None);
    let k2 = crate::imp::crypto::derive_decryption_key(canonical, None);
    assert_eq!(k1.len(), 32);
...
fn two_distinct_urns_yield_two_distinct_keys() {
    ...
    let ka = crate::imp::crypto::derive_decryption_key(a, None);
    let kb = crate::imp::crypto::derive_decryption_key(b, None);
    assert_ne!(
        ka, kb,
...
fn public_and_private_same_urn_yield_distinct_keys() {
    ...
    let pub_k = crate::imp::crypto::derive_decryption_key(u, None);
    let priv_k = crate::imp::crypto::derive_decryption_key(u, Some(&SecretSalt([0x09; 32])));
    assert_ne!(
        pub_k, priv_k,
...
fn two_private_salts_same_urn_yield_distinct_keys() {
    use crate::imp::core::SecretSalt;
    let u = "urn:dig:mainnet:2222222222222222222222222222222222222222222222222222222222222222/a";
    let k1 = crate::imp::crypto::derive_decryption_key(u, Some(&SecretSalt([0x01; 32])));
    let k2 = crate::imp::crypto::derive_decryption_key(u, Some(&SecretSalt([0x02; 32])));
