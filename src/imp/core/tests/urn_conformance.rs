...
//! (dig-capsule-core here, plus the dig-sdk / extension / browser ports) is expected to
...
fn fixture_chain_matches_canonical_chain_constant() {
    ...
    assert_eq!(
        set.canonical_chain,
        crate::imp::core::CHAIN,
        "the fixture's canonical chain must equal crate::imp::core::CHAIN"
    );
    assert_eq!(
        set.canonical_chain,
    ...
    assert_eq!(crate::imp::core::CHAIN, dig_urn_protocol::CANONICAL_CHAIN);
}
...
fn regenerate() {
    ...
    let doc = serde_json::json!({
        "_comment": "FROZEN normative URN conformance vectors (gap #128). Grammar: crate::imp::core::urn_grammar (URN_ABNF). Regenerate via `cargo test -p dig-capsule-core --test urn_conformance -- --ignored regenerate`. retrieval_key_hex = SHA-256(canonical) lowercase hex.",
        "canonical_chain": crate::imp::core::CHAIN,
        "valid": valid,
        "invalid": invalid,
    });
