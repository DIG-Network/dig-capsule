...
/// [`crate::imp::core::datasection::DIGS_DATA_OFFSET`] (2 MiB). The compiler injects
...
///
/// `result` is the canonical [`crate::imp::core::CompilationResult`] (its `stats`
/// field is the canonical [`crate::imp::core::CompilationStats`]). `detail` is the
...
impl Compiler {
    ...
    pub fn compile<G: GenerationView>(
        ...
        if blob_len_without_filler > config.uniform_blob_len {
            // Cannot happen under `crate::imp::core::MAX_STORE_BYTES` with the
