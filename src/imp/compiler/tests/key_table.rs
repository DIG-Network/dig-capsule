...
fn fixtures_build_and_expose_two_generations() {
    let gens = super::common::sample_generations();
    assert_eq!(gens.len(), 2);
    let rk = super::common::resource_key("about.html");
    assert!(gens
        .iter()
...
fn total_size_is_sum_of_chunk_body_lengths() {
    use super::common::{chunk, FakeGeneration, ResourceSpec};
...
fn verify_against_flags_out_of_range_index() {
    ...
    assert!(matches!(
        err,
        crate::imp::compiler::CompilerError::MissingChunk(2)
    ));
}
