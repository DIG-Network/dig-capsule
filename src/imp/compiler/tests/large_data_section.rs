...
//! `crate::imp::host::HostRuntime`, and asserts:
...
/// The rootless retrieval key as a `crate::imp::core::Bytes32`. Byte-identical to the
...
fn compile_and_extract_root(uniform_blob_len: usize, tag: &str) -> (u64, Bytes32) {
    let guest = read_guest_wasm();
    let store_id = crate::imp::core::sha256(&[0xCDu8; 48]); // SHA-256(pubkey) == store id
...
fn near_cap_store_serves_within_the_384mib_ceiling() {
    ...
    // (`crate::imp::compiler::FIXED_BLOB_LEN`) and served end-to-end through the
    ...
    serve_single_resource_and_verify(
        payload,
        "near-cap",
        crate::imp::compiler::FIXED_BLOB_LEN, // full 128 MiB production budget
    );
}
