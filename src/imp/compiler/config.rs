...
/// version 1."). A LITERAL `"1.0.0"`, DELIBERATELY DECOUPLED from the crate
/// version: `COMPILER_VERSION` is byte-recorded into every compile outcome
/// (SPEC §5), so it is a frozen artifact constant, not the package version. When
/// the members were collapsed into the single `dig-capsule` crate (v0.3.0, #1270)
/// the old `env!("CARGO_PKG_VERSION")` binding would have silently rewritten this
/// recorded byte string — a §5.1 custody break — so it is pinned to the literal.
/// The module-format-version half of the §5 claim is the DIGS blob header byte
/// (== 1, owned by `crate::imp::core::datasection`).
...
/// covers a max-cap store (`crate::imp::core::MAX_STORE_BYTES` = 128 MB = 122.07
/// MiB) ciphertext + key table + merkle + header with ~6 MiB headroom.
///
/// CANONICAL capsule-size relationship (#130): `crate::imp::core::MAX_STORE_BYTES`
...
const _: () = assert!(
    FIXED_BLOB_LEN as u64 >= crate::imp::core::MAX_STORE_BYTES,
    "FIXED_BLOB_LEN must be >= crate::imp::core::MAX_STORE_BYTES (the canonical \
...
pub struct CompilerConfig {
    ...
    /// `crate::imp::core::MAX_STORE_BYTES`.
...
/// with the canonical [`crate::imp::core::CompilationStats`] carried inside
/// [`crate::imp::core::CompilationResult`]).
